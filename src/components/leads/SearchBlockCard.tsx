...
export function SearchBlockCard({ block, index, canRemove, status = "idle", onChange, onRemove }: Props) {
  return (
    <div className="border border-border rounded-lg p-4 space-y-3 bg-card relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="space-y-1">
          ...
          <Input placeholder="ex: clínica odontológica" value={block.query} onChange={(e) => onChange(block.id, "query", e.target.value)} />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Cidade</Label>
          <Input placeholder="ex: São Paulo" value={block.cidade} onChange={(e) => onChange(block.id, "cidade", e.target.value)} />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Estado</Label>
          <Input placeholder="ex: SP" value={block.estado} onChange={(e) => onChange(block.id, "estado", e.target.value)} />
        </div>
        <div className="space-y-1">
