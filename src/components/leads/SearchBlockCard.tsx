...
import { Trash2, Loader2, CheckCircle2, XCircle, MapPin, AlertTriangle, Plus, X, Lightbulb, Info } from "lucide-react";
...
export function SearchBlockCard({ block, index, canRemove, status = "idle", result, onChange, onRemove, onDropData }: Props) {
  ...
  const subnichoCount = (block.subnichos || []).length;
  const bairroCount = block.bairros.length;
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="space-y-1">
            ...
            <Input placeholder="Ex: Saúde, Estética, Jurídico, Veterinária" value={block.query} onChange={(e) => onChange(block.id, "query", e.target.value)} className="bg-secondary border-border" />
            <p className="text-[11px] text-white/70">O nicho define o mercado principal da busca.</p>
          </div>
          {/* Cidade */}
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-white/70">Cidade onde a busca será realizada.</p>
          </div>
          {/* Estado */}
          <div className="space-y-1">
          ...
          {/* Quantidade */}
          <div className="space-y-1">
            <Label className="text-base font-bold text-white">Quantidade de leads</Label>
            <Select value={String(block.targetTotal)} onValueChange={(v) => onChange(block.id, "targetTotal", Number(v))}>
              ...
              <SelectContent>
                ...
                <SelectItem value="5000">5.000 leads — prospecção massiva</SelectItem>
                <SelectItem value="10000">10.000 leads — cobertura total</SelectItem>
              </SelectContent>
            </Select>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-white">Selecionados: {subnichoCount} / 10</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <Info className="h-3.5 w-3.5 text-white/60 cursor-help" />
              </TooltipTrigger>
              <TooltipContent>
                <p className="text-xs max-w-[200px]">Mais subnichos aumentam o número de empresas encontradas.</p>
              </TooltipContent>
            </Tooltip>
          </div>
          <p className="text-[11px] text-white/70">Selecione subnichos para ampliar a cobertura da busca sem perder precisão.</p>
                    ...
                    <Badge
                      ...
                      className={`cursor-pointer text-[10px] transition-colors ${
                        isAdded
                          ...
                          : "border-border text-white/70 hover:border-primary hover:text-primary"
                      }`}
                      onClick={() => {
                        if (isAdded) {
                          onChange(block.id, "subnichos", (block.subnichos || []).filter(x => normalizeKey(x) !== normalizeKey(s)));
                        } else if (subnichoCount < 10 && !hasSubnicho(block.subnichos || [], s)) {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-white">Selecionados: {bairroCount} / {MAX_BAIRROS}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <Info className="h-3.5 w-3.5 text-white/60 cursor-help" />
              </TooltipTrigger>
              <TooltipContent>
                <p className="text-xs max-w-[200px]">Se nenhum bairro for selecionado, a busca será feita na cidade inteira.</p>
              </TooltipContent>
            </Tooltip>
            {hasSuggestions && (
              <Button
          ...
          <p className="text-[11px] text-white/70">Selecione bairros específicos para refinar a prospecção.</p>
